...
const Login = () => {
  ...
  return (
    <div className ='flex items-center justify-center min-h-screen px-6 sm:px-0   bg-linear-to-br from-blue-200 to-purple-400 '>
        ...
        <img onClick={()=>navigate('/')} className='absolute left-5 sm:left-20 top-5 w-28 sm:w-32  md:w-48 cursor-pointer' src={assets.logo} alt="" />
        ...
        <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
            ...
            <form onSubmit={onSubmitHandler}>
              ...
              <p onClick={()=>navigate('/reset-password')} className='mt-4 text-white cursor-pointer text-center'>
                Forgot Password ?
              </p>

              <button className='mt-4 w-full py-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-900 font-medium text-white text-2l'>{state}</button>
            </form>
            ...
            : (
            <p className='text-gray-400 text-center text-xs mt-4'>
             Don't have an account ? {" "}<span onClick={()=>setState('Sign Up')} className='text-blue-400 cursor-pointer underline'>  Sign Up here</span>
            </p>)}
